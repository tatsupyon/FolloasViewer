...
*   **フォルダの直接起動**:
    ...
    *   `python FolloasViewer.py -no-compress-l2` のように引数を付けると、Live2画像の圧縮を無効化（中央クロップ）して起動します。

## 3. 解析表示とSCORE閾値
*   **解析位相の調整と固定 (Lock)**:
    *   **解析位相**の数値を変更することで、映像と認識ログのタイミングのズレ（位相）を調整できます。
    *   **[ログ固定 (Lock)]**: チェックを入れると、画面上の認識枠の表示がその時点のコマで固定されます。映像を再生・コマ送りすると、ズレを補正するために位相の数値が自動的に追従計算されます。特定のシーンでのログ合わせに活用してください。
*   **SCORE閾値の変更**:
...
## 5. 学習用データのエクスポート
Live2の映像を物体検出AIの学習データとして抽出することができます。
*   **[学習画像エクスポート]**: ボタンを押すと、現在表示中のLive2画像から認識対象領域（中央400x720）を切り出し、720x720に引き伸ばした画像（JPEG）を `training` フォルダに保存します。
*   **アノテーションファイル**: 画像と同時に、YOLOフォーマット（相対座標）に変換されたラベルデータ（`.txt`）も自動生成されます。
